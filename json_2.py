    ...
    def run(self):
        ...
        while running:
            ...
            if self.player_Cancel_bool:
                ...
                for j in range(0, player_create_count):
                    save_record(player_list[0].player_position, player_list[0].player_distance,
                                player_list[0].avg_speed,
                                player_list[0].max_speed, player_list[0].lead_distance, player_list[0].impormation,
                                player_list[0].pointList, player_list[0].route_pointList, record_num,
                                player_list[0].start_time,
                                player_list[0].player_distance_list)
                    player_delete(0)
...
def save_record(player_position, player_distance, avg_speed, max_speed, lead_distance, impormation, pointList, route_pointList, record_num, start_time, player_distance_list):
    # record_num = load_files_number("result/%s_result" % video_path.split('.')[0].split('\\')[-1] + "/player")
    ...
    if impormation["베이스"] == []:
        start_base_position = "No Data"
        end_base_position = "No Data"
    elif point_list.index(impormation["베이스"][0]) == 0:
        start_base_position = "2 base"
        end_base_position = "3 base"
    elif point_list.index(impormation["베이스"][0]) == 1:
        start_base_position = "1 base"
        end_base_position = "2 base"
    elif point_list.index(impormation["베이스"][0]) == 2:
        start_base_position = "Home base"
        end_base_position = "1 base"
    elif point_list.index(impormation["베이스"][0]) == 3:
        start_base_position = "3 Base"
        end_base_position = "Home base"

    player_data = OrderedDict()

    player_data["Player_Position"] = position
    player_data["Total_Distance"] = player_distance
    player_data["Average_Speed"] = avg_speed
    player_data["Max_Speed"] = max_speed
    player_data["Total_Running_time"] = round(cap.get(cv2.CAP_PROP_POS_MSEC) / 1000, 2) - start_time
    player_data["Lead_Distance"] = lead_distance
    if impormation["베이스"] == []:
        player_data["Section_Data"] = [{"Start_base": start_base_position, "Arrival_base": end_base_position, "Running_time": "No Data", "Running_distance": "No Data"}]
    if impormation["베이스"] != []:
        player_data["Section_Data"] = [{"Start_base": start_base_position, "Arrival_base": end_base_position, "Running_time": impormation["시간"][0], "Running_distance": impormation["거리"][0]}]
    player_data["Point_Data"] = [{"Point": pointList[0], "Speed": 0.0, "Avg_Speed": 0.0, "Video_Time": str(datetime.timedelta(seconds = round(start_time, 2))), "Momentary_Move_Distance": 0.0}]


    if impormation["베이스"] != []:
        for i in range(1, len(impormation["베이스"])):
            if point_list.index(impormation["베이스"][i]) == 0:
                start_base_position = "2 base"
                end_base_position = "3 base"
            elif point_list.index(impormation["베이스"][i]) == 1:
                start_base_position = "1 base"
                end_base_position = "2 base"
            elif point_list.index(impormation["베이스"][i]) == 2:
                start_base_position = "Home base"
                end_base_position = "1 base"
            elif point_list.index(impormation["베이스"][i]) == 3:
                start_base_position = "3 Base"
                end_base_position = "Home base"

            player_data['Section_Data'].append(
                {"Start_base": start_base_position, "Arrival_base": end_base_position, "Running_time": impormation["시간"][i], "Running_distance": impormation["거리"][i]}
            )

    sum_route_pointList = 0
    for i in range(0, len(pointList)):
        sum_route_pointList = route_pointList[i] + sum_route_pointList
        player_data['Point_Data'].append(
            {
                "Point": pointList[i], "Speed": route_pointList[i],
                "Avg_Speed": round(sum_route_pointList / (i + 1), 2),
                # "Video_Time": round(start_time + ((i + 1) * frame_num / cap.get(cv2.CAP_PROP_FPS)), 2),
                "Video_Time": str(datetime.timedelta(seconds = round(start_time + ((i + 1) * frame_num / cap.get(cv2.CAP_PROP_FPS)), 2))),
                "Momentary_Move_Distance": round(player_distance_list[i], 2)
            }
        )

    record_name = "result/%s_result" % video_path.split('.')[0].split('\\')[-1] + "/player/record_" + str(record_num) + ".json"
    with open(record_name, "a", encoding="utf-8") as json_file:
...
def player_tracking(player_order, frame_key, start):
    ...
    if count % frame_num == 0:
        ...
        player_list[player_order - 1].route_color(start, frame, frame_key, perspect_map, onepixel)
...
class Player():
    def __init__(self):
        ...
        self.pre_player_distance = 0
        self.momentary_move_distance = 0
        self.player_distance_list = []
        self.now_speed = 0
        self.max_speed = 0
        self.avg_speed = 0
        self.now_base = []
    ...
    def draw_box(self, number):
        ...
        if number == 0:
            return cv2.rectangle(img, pt1, pt2, (0, 0, 255), 2)
        elif number == 1:
            return cv2.rectangle(img, pt1, pt2, (255, 0, 0), 2)
        elif number == 2:
            return cv2.rectangle(img, pt1, pt2, (0, 255, 0), 2)
        elif number == 3:
            return cv2.rectangle(img, pt1, pt2, (0, 255, 255), 2)
    ...
    def mean_avg(self, start):
        ...
        if start == 1:
            ...
            if self.mean_avg_list.count([0, 0]) < self.mean_avg_list_size: # d
                ...
                if not self.pointList: # 비었으면 넣는다.
                    self.pointList.append(self.point_mean[0:2])
                elif abs(self.pointList[-1][0]-self.point_mean[0]) == 1 and abs(self.pointList[-1][1]- self.point_mean[1]) == 1:
                    self.pointList.append(self.point_mean[0:2])
                elif abs(self.pointList[-1][0]-self.point_mean[0]) == 1 and abs(self.pointList[-1][1]- self.point_mean[1]) > 1:
                    self.pointList.append(self.point_mean[0:2])
                elif abs(self.pointList[-1][0]-self.point_mean[0]) == 1 and abs(self.pointList[-1][1]- self.point_mean[1]) < 1:
                    self.pointList.append(self.pointList[-1])
                elif abs(self.pointList[-1][0]-self.point_mean[0]) > 1 and abs(self.pointList[-1][1]- self.point_mean[1]) == 1:
                    self.pointList.append(self.point_mean[0:2])
                elif abs(self.pointList[-1][0]-self.point_mean[0]) < 1 and abs(self.pointList[-1][1]- self.point_mean[1]) == 1:
                    self.pointList.append(self.pointList[-1])
                    ...
                    self.pointList.append(self.point_mean[0:2])
                # print("pointList.append", self.pointList)

        cv2.line(img, (self.adj_center_x, self.adj_center_y), (self.adj_center_x, self.adj_center_y), (255, 0, 255), 3)

    def route_color(self, start, frame, frame_key, perspect_map, onepixel):  # 이동경로를 색상으로 표현하기 위하여 구간별 속도 계산
        if start == 1:
            if frame_key + 1 == frame:
            ...
            # print("직전거리", self.pre_route_pers_distance)
            # 단위거리 = 총달린거리 - 직전달린거리
            if route_run_time != 0: # 순간속도
                route_v = round(abs(self.route_pers_distance - self.pre_route_pers_distance) / route_run_time * 3.6, 2)
                # print("route_v",route_v)

                self.momentary_move_distance = self.route_pers_distance - self.pre_route_pers_distance
                # print("momentary_move_distance", self.momentary_move_distance)
                self.player_distance_list.append(self.momentary_move_distance)


                self.avg_route_pointList.append(route_v)
                if len(self.avg_route_pointList) > 4:
                    self.avg_route_pointList.pop(0)
                # print("avg_route_pointList",self.avg_route_pointList)
                avg_route_pointList_sum = []
                div_constant = 0
                for i in range(0, len(self.avg_route_pointList)):
                    avg_route_pointList_sum.append(self.avg_route_pointList[i] * (i + 1))
                    div_constant = div_constant + (i + 1)
                weight_ma = round(sum(avg_route_pointList_sum) /div_constant, 2)

                self.route_pointList.append(weight_ma)
    ...
    def draw_route(self):
        ...
        if len(self.route_pointList) > 0:
            # avg_route = sum(self.route_pointList) / len(self.route_pointList)
            # self.avg_route_pointList.append(avg_route)
    ...
    # 통과 후 정보 측정하기
    def measure(self):
        running_time = round(round(cap.get(cv2.CAP_PROP_POS_MSEC) / 1000, 2) - self.start_time, 2)  # 달린시간 = 현재시간 - 출발시간
    ...
    def calculation_between_base(self):
        ...
        if (math.sqrt(pow(self.next_base[0] - self.nowPoint[0], 2) + pow(self.next_base[1] - self.nowPoint[1], 2))) < 20.0:
